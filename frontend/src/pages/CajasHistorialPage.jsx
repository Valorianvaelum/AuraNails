...
export default function CajasHistorialPage() {
  ...
  return <main className="min-h-screen bg-[#fff8f7] text-[#3d2f32]"><AppHeader /><section className="mx-auto max-w-5xl px-5 py-8 sm:px-8"><div className="flex flex-wrap justify-between gap-3"><div><Link className="aura-action aura-action-contextual" to="/caja">Volver a Caja</Link><h1 className="mt-4 text-3xl font-semibold">Historial de cajas</h1></div></div><div className="mt-5 grid gap-3 rounded-2xl border bg-white p-4 sm:grid-cols-3"><label className="grid gap-1 text-sm font-medium">Fecha<input type="date" value={fecha} onChange={(event) => setFecha(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium">Estado<select value={estado} onChange={(event) => setEstado(event.target.value)}><option value="">Todos</option><option value="abierta">Abiertas</option><option value="cerrada">Cerradas</option></select></label><label className="flex items-center gap-2 text-sm font-medium"><input checked={conDiferencia} type="checkbox" onChange={(event) => setConDiferencia(event.target.checked)} />Con diferencia</label>{tieneFiltros && <button className="aura-action aura-action-contextual justify-self-start" type="button" onClick={limpiar}>Limpiar filtros</button>}</div>{cargando && <p className="mt-5">Cargando historial...</p>}{error && <div className="mt-5 rounded-xl border border-[#e7c5ca] bg-white p-4"><p className="text-[#8b3f4c]">{error}</p><button className="mt-3 aura-action aura-action-secondary" type="button" onClick={cargar}>Reintentar</button></div>}{!cargando && !error && <div className="mt-5 grid gap-4">{visibles.map((caja) => <article className="aura-financial-card" key={caja.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">Caja #{caja.id}</h2><p className="text-sm text-[#6f5b60]">Apertura: {fechaHora(caja.abierta_en)}</p><p className="text-sm text-[#6f5b60]">Cierre: {fechaHora(caja.cerrada_en)}</p></div><span className={caja.estado === "cerrada" ? "rounded-full bg-[#f1e4e6] px-2 py-1 text-sm font-semibold text-[#8b3f4c]" : "rounded-full bg-[#e7f5ea] px-2 py-1 text-sm font-semibold text-[#356640]"}>{caja.estado_display}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><p className="aura-financial-summary text-sm">Inicial: <strong>{dinero(caja.saldo_inicial)}</strong></p><p className="aura-financial-summary text-sm">Esperado: <strong>{dinero(caja.resumen?.saldo_teorico ?? caja.saldo_teorico_cierre)}</strong></p><p className="aura-financial-summary text-sm">Contado: <strong>{caja.saldo_contado === null ? "Pendiente" : dinero(caja.saldo_contado)}</strong></p><p className="aura-financial-summary text-sm">Diferencia: <strong>{caja.diferencia === null ? "Pendiente" : dinero(caja.diferencia)}</strong></p></div><Link className="mt-4 aura-action aura-action-secondary" to={`/caja/${caja.id}`}>Ver detalle</Link></article>)}{!visibles.length && <p className="aura-financial-empty text-center">{tieneFiltros ? "No encontramos cajas con los filtros seleccionados." : "Todavía no tenés cajas registradas."}</p>}</div>}</section></main>;
}
