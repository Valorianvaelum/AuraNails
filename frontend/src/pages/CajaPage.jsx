  ...
  return (
    <main className="min-h-screen bg-[#fff4f7] text-[#3d2f32]">
      ...
      <section className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold">Caja</h1><p className="mt-1 text-[#6f5b60]">Controlá el dinero físico de la jornada.</p></div><Link className="font-semibold underline underline-offset-4" to="/caja/historial">Ver historial</Link></div>
        ...
        {!cargando && !error && caja && <><section className="mt-6 rounded-3xl border border-[#efdadd] bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><span className="rounded-full bg-[#e7f5ea] px-3 py-1 text-sm font-semibold text-[#356640]">Caja abierta</span><p className="mt-4 text-sm text-[#6f5b60]">Abierta: {fechaHora(caja.abierta_en)}</p><p className="mt-1 text-sm text-[#6f5b60]">Saldo inicial: {dinero(caja.saldo_inicial)}</p></div><Link className="font-semibold underline underline-offset-4" to={`/caja/${caja.id}`}>Ver detalle</Link></div><div className="mt-6"><CajaResumen caja={caja} /></div><div className="mt-6 flex flex-wrap gap-3 border-t border-[#e5dce2] pt-5"><button className="rounded-xl border border-[#f1dce4] bg-white px-4 py-2 font-semibold transition hover:bg-[#fff4f7]" type="button" onClick={() => setDialogo({ tipo: "gasto" })}>Registrar gasto</button><button className="rounded-xl border border-[#f1dce4] bg-white px-4 py-2 font-semibold transition hover:bg-[#fff4f7]" type="button" onClick={() => setDialogo({ tipo: "aporte" })}>Registrar aporte</button><button className="rounded-xl border border-[#f1dce4] bg-white px-4 py-2 font-semibold transition hover:bg-[#fff4f7]" type="button" onClick={() => setDialogo({ tipo: "retiro" })}>Registrar retiro</button><button className="rounded-xl bg-[#8b3f4c] px-4 py-2 font-semibold text-white transition hover:bg-[#71323d]" type="button" onClick={() => setDialogo({ tipo: "cerrar" })}>Cerrar caja</button></div></section><CajaMovimientos caja={caja} onAnularGasto={(registro) => setDialogo({ tipo: "anularGasto", registro })} onAnularMovimiento={(registro) => setDialogo({ tipo: "anularMovimiento", registro })} /></>}
      </section>
