...
function TarjetaTurno({ turno, resumida = false }) {
  ...
  return <article className="rounded-2xl border border-[#f1dce4] bg-white p-4">
...
export default function AgendaPage() {
  ...
  return <main className="min-h-screen bg-[#fff4f7] text-[#3d2f32]"><AppHeader /><section className="mx-auto max-w-7xl px-5 py-8 sm:px-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#b76e79]">Organización de turnos</p><h1 className="mt-2 text-3xl font-semibold">Agenda</h1><p className="mt-1 capitalize text-[#6f5b60]">{vista === "dia" ? fechaLarga(desde) : `${fechaLarga(desde)} al ${fechaLarga(hasta)}`}</p></div><Link className="rounded-xl bg-[#b76e79] px-4 py-2 font-semibold text-white" to={nuevoTurno}>Nuevo turno</Link></div>
    <section className="mt-6 rounded-2xl border border-[#f1dce4] bg-white p-4" aria-label="Controles de agenda"><div className="flex flex-wrap items-end gap-2"><div className="flex rounded-xl bg-[#fff4f7] p-1" role="group" aria-label="Vista de agenda"><button className={`rounded-lg px-3 py-2 text-sm font-semibold ${vista === "dia" ? "bg-white text-[#7d4e57] shadow-sm" : "text-[#6f5b60]"}`} type="button" onClick={() => setVista("dia")}>Día</button><button className={`rounded-lg px-3 py-2 text-sm font-semibold ${vista === "semana" ? "bg-white text-[#7d4e57] shadow-sm" : "text-[#6f5b60]"}`} type="button" onClick={() => setVista("semana")}>Semana</button></div><button className="rounded-xl border border-[#f1dce4] px-3 py-2 text-sm font-semibold" type="button" onClick={() => mover(-1)}>Anterior</button><button className="rounded-xl border border-[#f1dce4] px-3 py-2 text-sm font-semibold" type="button" onClick={() => setFecha(hoy())}>Hoy</button><button className="rounded-xl border border-[#f1dce4] px-3 py-2 text-sm font-semibold" type="button" onClick={() => mover(1)}>Siguiente</button><label className="ml-auto grid gap-1 text-sm font-medium">Fecha<input className="rounded-xl border border-[#f1dce4] px-3 py-2" type="date" value={fecha} onChange={(event) => setFecha(event.target.value)} /></label></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><label className="grid gap-1 text-sm font-medium">Estado<select className="rounded-xl border border-[#f1dce4] px-3 py-2" value={estado} onChange={(event) => setEstado(event.target.value)}><option value="">Todos</option>{ESTADOS.map(([valor, etiqueta]) => <option value={valor} key={valor}>{etiqueta}</option>)}</select></label><label className="grid gap-1 text-sm font-medium">Clienta<select className="rounded-xl border border-[#f1dce4] px-3 py-2" value={clientaId} onChange={(event) => setClientaId(event.target.value)}><option value="">Todas las clientas</option>{clientas.map((clienta) => <option value={clienta.id} key={clienta.id}>{clienta.nombre_completo}</option>)}</select>{errorClientas && <span className="text-xs text-[#8b3f4c]">{errorClientas}</span>}</label><label className="grid gap-1 text-sm font-medium">Buscar clienta<input className="rounded-xl border border-[#f1dce4] px-3 py-2" placeholder="Nombre o teléfono" value={busqueda} onChange={(event) => setBusqueda(event.target.value)} /></label></div>{tieneFiltros && <button className="mt-4 text-sm font-semibold underline" type="button" onClick={() => { setEstado(""); setClientaId(""); setBusqueda(""); }}>Limpiar filtros</button>}</section>
